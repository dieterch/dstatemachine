...
def bokeh_chart(source, pltcfg, x_ax='datetime', x_ax_unit=None, title=None, grid=True, legend=True, style='line', x_range=None, y_range=None, figsize=(8,6), *args, **kwargs):
    ...
    for i, y in enumerate(pltcfg):
        ...
        unit=[]
        renderers=[]
        legend_label_group = y.get('label', None)
        for ii, col in enumerate(y['col']):
            ...
            lbl = legend_label_group if legend_label_group else col
            if legend:
                if style == 'line':
                    func = getattr(p, 'line')
                    r = func(source=source, x=x_ax, y=col,
                        color=color, y_range_name=str(i), legend_label=lbl, line_width=linewidth, name=col)
                    renderers.append(r)
                if style == 'circle':
                    func = getattr(p, 'scatter')
                    r = func(source=source, x=x_ax, y=col,
                        color=color, y_range_name=str(i), legend_label=lbl, size=6, name=col)
                    renderers.append(r)
                if style == 'both':
                    func = getattr(p, 'line')
                    r = func(source=source, x=x_ax, y=col,
                ...
                if style == 'line':
                    func = getattr(p, 'line')
                    r = func(source=source, x=x_ax, y=col,
                        color=color, y_range_name=str(i), line_width=linewidth, name=col)
                    renderers.append(r)
                if style == 'circle':
                    func = getattr(p, 'scatter')
                    r = func(source=source, x=x_ax, y=col,
                        color=color, y_range_name=str(i), size=6, name=col)
                    renderers.append(r)
                if style == 'both':
                    func = getattr(p, 'line')
                    r = func(source=source, x=x_ax, y=col,
                    ...
                    r = func(source=source, x=x_ax, y=col,
                        color=color, y_range_name=str(i), size=6, name=col)
                    renderers.append(r)
            tooltips.append((col, '@{'+col+'}{0.2 f} '+unit[-1]))
        ...
        label_cols = [y['label']] if 'label' in y else y['col']
        if len(unit)==1 or unit.count(unit[0]) == len(unit): #if only one entry or all have the same unit
            llabel = ', '.join(label_cols)+' ['+unit[0]+']'
        else:
            llabel = ', '.join(label_cols)+' ['+', '.join(unit)+']'
    ...
    # One HoverTool per data renderer so Bokeh only shows it when the renderer is visible
    all_renderers = [r for r in p.renderers if hasattr(r, 'name') and r.name]
    seen_cols = set()
    for r in all_renderers:
        col = r.name
        if col in seen_cols:  # skip the scatter duplicate in 'both' style
            continue
        seen_cols.add(col)
        tip = tooltips[0:1] + [(t[0], t[1]) for t in tooltips[1:] if t[1].startswith('@{'+col+'}')]
